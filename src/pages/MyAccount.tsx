...
export function MyAccount() {
  ...
  const [active,setActive]=useState<'summary'|'orders'|'profile'|'security'|'notifications'|'favorites'|'recent'|'materials'>(()=>searchParams.get('seccion')==='notificaciones'?'notifications':searchParams.get('seccion')==='profile'?'profile':searchParams.get('seccion')==='materiales'?'materials':'summary')
  ...
  const passwordUpdate = useMutation({ mutationFn:changePassword })
  ...
  const submitPassword = (event:FormEvent<HTMLFormElement>) => {
    ...
    const passwordNueva=String(data.get('password_nueva'))
    const confirmarPassword=String(data.get('confirmar_password'))
    setPasswordError('')
    passwordUpdate.reset()
    if(passwordNueva!==confirmarPassword){setPasswordError('Las contraseñas no coinciden.');return}
    passwordUpdate.mutate({password_actual:String(data.get('password_actual')),password_nueva:passwordNueva,confirmar_password:confirmarPassword},{onSuccess:()=>form.reset()})
  }
  ...
  return <main className="min-h-screen bg-[#f7f7f5] text-neutral-950">
    ...
    <div className="mx-auto max-w-360 space-y-10 px-5 py-12 sm:px-8 lg:px-[7vw] lg:py-16">
      ...
      {active==='notifications'&&<section className="rounded-none border border-neutral-200 bg-white p-5"><h2 className="font-bold">Notificaciones {notifications.data&&notifications.data.no_leidas>0&&<span className="ml-2 border border-black bg-black px-2 py-1 text-xs text-white">{notifications.data.no_leidas}</span>}</h2><div className="mt-3 grid gap-2">{notifications.data?.items.map(item=><article className={`rounded-none p-3 text-left text-sm ${item.leida?'bg-gray-50':'bg-neutral-50'}`} key={item.id}><button className="w-full text-left" onClick={()=>!item.leida&&markRead.mutate(item.id)}><strong>{item.titulo}</strong><span className="mt-1 block text-gray-600">{item.mensaje}</span><small className="mt-2 block text-gray-400">{new Date(item.creada_en).toLocaleString('es-AR')}</small></button></article>)}{!notifications.data?.items.length&&<p className="p-6 text-center text-sm text-gray-500">No tenés notificaciones.</p>}</div></section>}
        ...
        {active==='security'&&<section className="max-w-xl border border-neutral-200 bg-white p-6"><h2 className="font-serif text-3xl font-semibold">Seguridad</h2><p className="mt-2 text-sm leading-6 text-neutral-500">Actualizá la contraseña con la que ingresás a tu cuenta.</p><form className="mt-6 space-y-4" onSubmit={submitPassword}>
          <label className="block text-xs font-bold">Contraseña actual<input className={field} name="password_actual" type="password" required autoComplete="current-password"/></label>
          <label className="block text-xs font-bold">Nueva contraseña<input className={field} name="password_nueva" type="password" required minLength={8} maxLength={128} autoComplete="new-password"/><small className="mt-1 block font-normal text-neutral-500">Debe tener al menos 8 caracteres.</small></label>
          <label className="block text-xs font-bold">Repetir nueva contraseña<input className={field} name="confirmar_password" type="password" required minLength={8} maxLength={128} autoComplete="new-password"/></label>
          {passwordUpdate.isSuccess&&<p className="bg-emerald-50 p-3 text-xs font-bold text-emerald-700">Contraseña actualizada correctamente.</p>}
          {passwordError&&<p className="bg-red-50 p-3 text-xs text-red-700">{passwordError}</p>}
          {passwordUpdate.isError&&<p className="bg-red-50 p-3 text-xs text-red-700">{passwordUpdate.error.message}</p>}
          <button className="w-full bg-black p-3 text-sm font-extrabold text-white disabled:opacity-50" disabled={passwordUpdate.isPending}>{passwordUpdate.isPending?'Actualizando…':'Cambiar contraseña'}</button>
        </form></section>}
